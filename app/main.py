...
async def train():
    ...
    try:
        ...
        model = GradientBoostingClassifier(learning_rate=0.1, max_depth=5,  min_samples_leaf=3,
...
async def prediction(request: PredictionValues):
    ...
    if model is None:
        raise HTTPException(status_code=400, detail="Model not trained. Use the /train endpoint first.")
    
    try:
        scaler = joblib.load("scaler.pkl")
        inp = [[request.Hydraulic_Pressurebar,
        ...
        confidence = max(model.predict_proba(inp)[0])  
